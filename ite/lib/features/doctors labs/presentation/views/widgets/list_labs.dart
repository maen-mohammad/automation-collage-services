...
class ListLabs extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<GetLabsCubit, GetLabsState>(
      builder: (context, state) {
        if (state is GetLabsLoading) {
          return Center(
            child: CircularProgressIndicator(),
          );
        } else if (state is GetLabsFailure) {
          return Center(
            child: Text(state.errormsg),
          );
        } else if (state is GetLabsSuccess) {
          return Container(
            height: 140,
            ...
            child: ListView.builder(
              ...
              itemCount: state.labsModel.labs?.length,
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            ...
                            LabName(text: state.labsModel.labs![index].name!)
                          ],
                        ),
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                            ...
                            GestureDetector(
                              onTap: () {
                                showCupertinoModalPopup<void>(
                                  context: context,
                                  builder: (BuildContext context) => LabDetails(
                                    lab: state.labsModel.labs![index],
                                  ),
                                );
                              },
                              child: AddButton(
                              ...
                              onTap: () {
                                showCupertinoModalPopup<void>(
                                  ...
                                  builder: (BuildContext context) =>
                                      Maintenanse(
                                    name: state.labsModel.labs![index].name!,
                                  ),
                                );
                              },
                              child: AddButton(
